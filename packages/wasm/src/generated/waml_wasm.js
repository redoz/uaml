 ...
 * @returns {any}
 */
export function fmt(bundle) {
    const ret = wasm.fmt(bundle);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
 ...
 * @returns {string}
 */
export function new_diagram_doc(kind, name) {
    let deferred3_0;
    let deferred3_1;
    try {
        ...
        const ret = wasm.new_diagram_doc(ptr0, len0, ptr1, len1);
        deferred3_0 = ret[0];
        deferred3_1 = ret[1];
 ...
 * @returns {any}
 */
export function split_bundle(text) {
    const ptr0 = passStringToWasm0(text, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.split_bundle(ptr0, len0);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('waml_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
