//! Thin WASM bindings over the WAML core. Each `#[wasm_bindgen]` entry point is a
...
/// the resolved [`Bundle`](waml::okf::Bundle) as JSON. Additive to
/// [`build_model_json`]: domain-agnostic, lossless, and it does not touch the
/// UML `Model` shape.
pub fn build_bundle_json(bundle: &[(String, String)]) -> String {
    serde_json::to_string(&waml::okf::build_bundle(bundle)).unwrap()
}

pub fn validate_json(bundle: &[(String, String)]) -> String {
    serde_json::to_string(&waml::validate::validate(bundle)).unwrap()
}
...
pub fn apply_ops_bundle(
    ...
    let dtos: Vec<waml_ops_dto::OpDto> =
        serde_json::from_str(ops_json).map_err(|e| e.to_string())?;
    let ops = dtos_to_ops(dtos)?;
    waml::ops::apply(bundle, &ops).map_err(|e| format!("op {}: {}", e.index, e.reason))
}

fn dtos_to_ops(dtos: Vec<waml_ops_dto::OpDto>) -> Result<Vec<waml::ops::Op>, String> {
...
pub fn fmt_bundle(bundle: &[(String, String)]) -> Vec<(String, String)> {
    bundle
        ...
        .map(|(p, t)| {
            (
                p.clone(),
                waml::serialize::serialize_document(&waml::parse::parse_document(t)),
            )
        })
        .collect()
}
...
pub fn reindex_bundle_core(bundle: &[(String, String)]) -> Vec<(String, String)> {
    waml::index_md::reindex_bundle(bundle)
}
...
pub fn solve_bundle(
    bundle: &[(String, String)],
    diagram_key: &str,
    sizes: waml::solve::SizeMap,
    cfg: waml::solve::SolveConfig,
...
pub fn build_bundle(bundle: JsValue) -> Result<JsValue, JsValue> {
    use serde::Serialize;
    let b: Vec<(String, String)> = serde_wasm_bindgen::from_value(bundle)?;
    let out = waml::okf::build_bundle(&b);
...
pub fn solve(
    ...
    let b: Vec<(String, String)> = serde_wasm_bindgen::from_value(bundle)?;
    let sizes: waml::solve::SizeMap = serde_wasm_bindgen::from_value(sizes)?;
    let cfg: waml::solve::SolveConfig = if cfg.is_null() || cfg.is_undefined() {
